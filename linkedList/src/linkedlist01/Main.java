...
//1번에 대한 답: 1) array 장점 :  논리적 순서와 물리적 순서가 일치한다.. 따라서 (index 값)을 통한 원소 (접근이 용이)하며, 구현이 쉽다
//              array 단점  : 삽입, 삭제 등 연산에 필요한 Cost가 높다는 것이다. (삽입, 삭제)의 경우 순서를 맞추기 위해 (이동하는 원소)가 많아진다. 

//            2) linked list: 논리적 순서대로 되어 있지만 물리적으로는 순서대로 되어 있지 않다. 각 원소가 (다음 index 위치)에 해당하는 물리적 주소를 가지고 있다.
//          삽입, 삭제의 경우  array랑 다르게 앞뒤 링크만 변경되고 나머지 링크는 변경되지 않는다
//         linked list 단점:  추가나 삭제가 용이하지만 (인덱스가 없기 때문에) 특정 요소에 접근하기 위해서는 (순차 탐색)이 필요. (인덱스값으로 바로 찾지 못하고 속도가 떨어진다). 
